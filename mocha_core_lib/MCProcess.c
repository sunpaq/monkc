...
constructor(MCProcess, xxx)
{
	link_class(MCProcess, MCObject, nil)
	{
		...
		have_method(MCProcess, fork, xxx) returns(0 in child/child-pid in parent/-1 on error);
		have_method(MCProcess, vfork, xxx) returns(0 in child/child-pid in parent/-1 on error);

		have_method(MCProcess, registerAtExitCallback, _FunctionPointer(func));
		have_method(MCProcess, exitWithStatus, int status);
		have_method(MCProcess, waitAnyChildExit, int* statusAddr) returns(pid_t);
		have_method(MCProcess, waitPIDChildExit, pid_t pid, int* statusAddr, MCProcessOptions options) returns(pid_t);

		have_method(MCProcess, isChildExitNormal, int status) returns(BOOL);
		have_method(MCProcess, getChildExitLowOrder8Bit, int status) returns(int);

		have_method(MCProcess, isChildExitBySignal, int status) returns(BOOL);
		have_method(MCProcess, getChildTerminateSignal, int status) returns(int signalNum);
		have_method(MCProcess, isCoreDumpFileGenerated, int status) returns(BOOL);

		have_method(MCProcess, isChildStopped, int status) returns(BOOL);
		have_method(MCProcess, getChildStopSignal, int status) returns(int signalNum);
		have_method(MCProcess, isChildContinued, int status) returns(BOOL);

		have_method(MCProcess, waitAnyChildExitGetResourceUseage, 
			int* statusAddr, int options, MCProcessRUseage* useage) returns(pid_t);

		have_method(MCProcess, waitPIDChildExitGetResourceUseage, pid_t pid,
			int* statusAddr, int options, MCProcessRUseage* useage) returns(pid_t);
	}
